    ...
    if result['yaml_updates'] and not dry_run and docket_file.exists():
        try:
            with open(docket_file, 'w', encoding='utf-8') as f:
                yaml.dump(yaml_entries, f, default_flow_style=False, allow_unicode=True, sort_keys=False)
        except Exception as e:
...
def main():
    ...
    for case_dir in case_dirs:
        ...
        if result['renames']:
            cases_modified += 1
            status = f"{Colors.YELLOW}⚡{Colors.RESET}" if not args.dry_run else f"{Colors.CYAN}📝{Colors.RESET}"
    ...
    print(f"  Files renamed:      {Colors.GREEN if total_renames else ''}{total_renames}{Colors.RESET}")
    print(f"  YAML paths updated: {Colors.GREEN if total_yaml_updates else ''}{total_yaml_updates}{Colors.RESET}")
