    ...
    if len(unique_entries) < len(entries) and not dry_run:
        try:
            with open(docket_file, 'w', encoding='utf-8') as f:
                yaml.dump(unique_entries, f, default_flow_style=False, allow_unicode=True, sort_keys=False)
            result['written'] = True
        except Exception as e:
...
def main():
    ...
    for docket_file in files:
        ...
        if result['removed']:
            status = f"{Colors.GREEN}✓{Colors.RESET}" if result.get('written') else f"{Colors.CYAN}📝{Colors.RESET}"
            print(f"{status} {result['case_id']}: {result['original_count']} → {result['final_count']} entries ({len(result['removed'])} duplicates removed)")
            
            for dup in result['removed'][:3]:
                print(f"    {Colors.RED}- {dup['entry_id']} (duplicate of {dup['kept_id']}){Colors.RESET}")
    ...
    print(f"  Duplicates removed:  {Colors.GREEN if total_removed else ''}{total_removed}{Colors.RESET}")
    print(f"{Colors.BOLD}───────────────────────────────────────────────────────────────{Colors.RESET}")
