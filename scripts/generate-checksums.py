...
def main():
    ...
    parser.add_argument('--manifest', action='store_true', help='Also create checksum manifest files')
    ...
    for docket_file in files:
        ...
        if result['updated'] > 0:
            files_modified += 1
            status = f"{Colors.YELLOW}⚡{Colors.RESET}" if result.get('written') else f"{Colors.CYAN}📝{Colors.RESET}"
            print(f"{status} {result['case_id']}: {result['updated']}/{result['total']} checksums generated")
        else:
            if result['already_set'] == result['total']:
                print(f"{Colors.GREEN}✓{Colors.RESET} {result['case_id']}: All {result['total']} checksums already set")
            else:
                print(f"{Colors.BLUE}○{Colors.RESET} {result['case_id']}: {result['total']} entries")
        
        if result['missing_files']:
            for mf in result['missing_files'][:2]:
                print(f"    {Colors.YELLOW}⚠ Missing: {mf}{Colors.RESET}")
            if len(result['missing_files']) > 2:
                print(f"    {Colors.YELLOW}... and {len(result['missing_files']) - 2} more missing{Colors.RESET}")
        ...
        if args.manifest and not args.dry_run:
            ...
            if manifest_result.get('entries', 0) > 0:
                print(f"    {Colors.CYAN}📄 Manifest: {manifest_result['entries']} entries{Colors.RESET}")
    ...
    print(f"  Checksums generated:  {Colors.GREEN if total_updated else ''}{total_updated}{Colors.RESET}")
    print(f"  Missing PDF files:    {Colors.YELLOW if total_missing else ''}{total_missing}{Colors.RESET}")
