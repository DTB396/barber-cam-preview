...
def main():
    ...
    for docket_file in docket_files:
        ...
        if result['unmatched']:
            print(f"    {Colors.RED}⚠ {len(result['unmatched'])} entries have no matching file{Colors.RESET}")
    ...
    print(f"  Still unmatched:   {Colors.RED if total_unmatched else ''}{total_unmatched}{Colors.RESET}")
