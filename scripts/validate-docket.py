...
def load_schema(schema_path: Path) -> dict:
    ...
    if not schema_path.exists():
        print(f"{Colors.YELLOW}⚠ Schema not found at {schema_path}, using built-in schema{Colors.RESET}")
    ...
    return {
        ...
        "properties": {
            ...
            "type": {"type": "string", "enum": [
                "Order", "Motion", "Brief", "Filing", "Notice", 
                "Opinion", "Judgment", "Subpoena", "Summons",
                "Complaint", "Answer", "Discovery", "Exhibit", 
                "Transcript", "Other"
            ]},
            "title": {"type": "string"},
            "file": {"type": "string"},
    ...
    if file_path:
        ...
        if not file_path.startswith('/assets/cases/'):
            if file_path.startswith('/cases/'):
                errors.append(f"Invalid path format: {file_path} (should start with /assets/cases/)")
            else:
...
def print_results(results: List[Dict], duplicates: List[str], strict: bool = False):
    ...
    for result in results:
        ...
        if len(result['warnings']) > 3:
            print(f"    {Colors.YELLOW}  ... and {len(result['warnings']) - 3} more warnings{Colors.RESET}")
    ...
    print(f"  Errors:         {Colors.RED if total_errors else Colors.GREEN}{total_errors}{Colors.RESET}")
    print(f"  Warnings:       {Colors.YELLOW if total_warnings else Colors.GREEN}{total_warnings}{Colors.RESET}")
    print(f"{Colors.BOLD}───────────────────────────────────────────────────────────────{Colors.RESET}")
